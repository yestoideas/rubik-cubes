...
const SecondLayerLayout = () => {
    ...
    const handleAnalysisData = () => {
        const newData = [...sectorsDetails].map((sector) => {
            const sectorData = sector.overviews.filter(item => item.title === selectedShortText.slice(0, 2));
            return {
                ...sector,
    ...
    const handleSelectAnalysis = (payload) => {
        ...
        const sectorData = payload?.overviews?.filter(item => item.title === selectedShortText.slice(0, 2));
                ...
                >
                    ...
                    {countryStep === 0 ?
                        <>
                                ...
                                <>
                                    <Stack direction={'row'}  alignItems={'center'} spacing={2}>
                                        {selectedShortText?.split('')?.map((item, i) => (
                                            <>
                                                <Typography lineHeight={1} key={`${item}`} variant='h6' fontWeight={600} >
                                                    {/* {item} */}
                                                    {cubeDetails[item]?.title}
                                                </Typography>

                                                {i < (selectedShortText.length - 1) ? <Divider sx={{ borderWidth: "1px", height: "100%", borderColor: "gray" }} /> : null}
                                            </>
                                        ))}
                                    </Stack>
                            ...
                            <Stack direction={'row'} spacing={3} justifyContent={'flex-start'} sx={{ marginBottom: '' }}>
                                {/* {!Object.keys(analysisData).length > 0 ? */}
                                {step < 1 ?
                                    <>
                                        <Button onClick={handleAnalysisData} variant="contained" color="success" sx={{ textTransform: 'none', fontWeight: 'bold', py: '0.85rem' }} >Run sector analysis</Button>

                                        <Button onClick={handleCountryData} variant="contained" color="success" sx={{ textTransform: 'none', fontWeight: 'bold', py: '0.85rem' }} >Run country analysis</Button>
                                    </>
                                    : null
                                }
                            </Stack>
